...
import OBR, { buildImage, buildText, isImage } from "@owlbear-rodeo/sdk";
...
export const clearCombatantCache = () => combatantCache = {};
	...
	for (const combatantName in combatants) {
		...
		for (const item of items) {
			if (item.text.plainText.toLowerCase().replaceAll(" ", "") === combatantName.toLowerCase().replaceAll(" ", "")) {
				...
				if (combatant.ac !== combatantCache[combatantName]?.ac)
					await updateAC(item, boundingBox, combatant, dpiScale);
				if (combatant.hp !== combatantCache[combatantName]?.hp || combatant.maxHp !== combatantCache[combatantName]?.maxHp)
					await updateHealth(item, boundingBox, combatant, dpiScale);
				if (combatant.thp !== combatantCache[combatantName]?.thp)
					await updateThp(item, boundingBox, combatant, dpiScale);
				if (combatant.hpStatus !== combatantCache[combatantName]?.hpStatus)
					await updateHealthStatus(item, boundingBox, combatant, dpiScale);
				if (combatant.conditions !== combatantCache[combatantName]?.conditions)
